            ...
            (when (and (equal operatorFlag 0))
                ...
                (when (and (equal valueFlag 1) (or (equal (elt word (1+ i)) " ") (equal (elt word (1+ i)) ")") (equal (elt word (1+ i)) "\"")))
                    ...
                    (when (every #'alpha-char-p (elt word (- i counter)))
                        (setq returnedList (append returnedList (list (list (concatString identiferListTemp) "LEXICAL ERROR: IDENTIFIER CONTAINS A NUMBER")))) 
                ...
                (when (and (equal identifierFlag 1) (or (equal (elt word (1+ i)) " ") (equal (elt word (1+ i)) ")") (equal (elt word (1+ i)) "\"")))
                    ...
                    (cond 
                        (strFlag (setq returnedList (append returnedList (list (list (concatString identiferListTemp) "VALUESTR")))) )
                        (t (setq returnedList (append returnedList (list (list (concatString identiferListTemp) "IDENTIFIER")))) )
                    )
