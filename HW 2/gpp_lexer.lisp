    ...
    (loop for i from 0 to (1- (length word))
        ...
        do (loop for j from 0 to (1- (length operators))
            do (when (equal (elt word i) (elt operators j))
                (cond 
                    ((and (equal (elt word i) "*") (equal (elt word (1+ i)) "*")) (format t "~a~%" "OP_DBLMULT") (setf flag nil)(setq operatorFlag 1))
                    ((and (equal (elt word i) "*") (equal (elt word (1- i)) "*")) (setf flag nil))
                    ((and (equal (elt word i) "/") (equal (elt word (1+ i)) "/")) (format t "~a~%" "OP_DIV2") (setf flag nil)(setq operatorFlag 1))
                    ((and (equal (elt word i) "/") (equal (elt word (1- i)) "/")) (setf flag nil))
                    ((equal (elt word i) "\"") (setq cCounter (1+ cCounter)) (when (equal (mod cCounter 2) 0) (format t "~a~%" "OP_CC")) (when (equal (mod cCounter 2) 1) (format t "~a~%" "OP_OC")) (setf flag nil)(setq operatorFlag 1))
                    (flag (format t "~a~%" (elt printOperators j))(setq operatorFlag 1))
                )    
            )
        )
